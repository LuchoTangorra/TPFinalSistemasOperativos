...
def model_fitter(batch_size, optimizer, model, epochs=100):
    ...
    wandb.init(project="cnn-eager-so-tensorflow-gpu", name='model {}, batch {}, optimizer{}'.format(model[2], batch_size, optimizer), reinit=True)

    model = Model(inputs=model[0], outputs=model[1])
    model.summary()
    model.compile(loss='binary_crossentropy', optimizer=optimizer, metrics=['binary_accuracy'], run_eagerly=True)
